...
public class WebApiStartup
{
	...
	private HttpConfiguration ConfigureWebApi()
	{
		...
		traceWriter.TraceSource = new TraceSource("text");
...
//		}
	...
	protected void WriteRequest(HttpRequestMessage request)
	{
		...
		var newLine = string.Format($"{request.Method.Method},{request.RequestUri}");
		csv.AppendLine(newLine);
		File.AppendAllText(@"C:\TEMP\log.csv", csv.ToString());
	}
	...
	protected void WriteTrace(TraceRecord rec)
	{
		...
		var newLine = string.Format($"{rec.Timestamp.ToLocalTime()},{rec.Category},{rec.Level},{rec.Kind},{rec.Status},{rec.Operator},{rec.Operation},{rec.Message}");
		csv.AppendLine(newLine);
